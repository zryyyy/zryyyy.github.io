...
`;
  ...
  fn.search = async () => {
    ...
    if (results === "") {
      results = `<div id="resule-hits-empty"><i class="fa-duotone fa-box-open"></i><p>${volantis.GLOBAL_CONFIG.languages.search.hits_empty.replace(/\$\{query}/, fn.queryText)}</p></div>`
    }
